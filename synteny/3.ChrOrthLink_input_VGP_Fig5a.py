...
fAlpha = 0.1
nRot_annot = "45"
iPATH_chrsize= "input/chrsize/"


def reordering_matrix_by_sID_list(fNAME, sID_LIST):
	oNAME = "reordered_"+os.path.basename(fNAME)
	fpout = open(oNAME,'w')
	nHeader = "BuscoID"
	for sID in sID_LIST:
		nHeader+=",BUSCO_"+sID
...
reordering_matrix_by_sID_list("Core_Matrix_BUSCO.csv",sID_LIST)
reordering_matrix_by_sID_list("Full_Matrix_BUSCO.csv",sID_LIST)

#nColorPalette = "rainbow"
# better to re-order the columns of BUSCO results depend on the order you want to show
try:
	fNAME_BuscoCoreMatrix = "reordered_Core_Matrix_BUSCO.csv"
	fpin = open(fNAME_BuscoCoreMatrix,'r')
	fpin.close()
except:
	try:
		fNAME_BuscoCoreMatrix = "Core_Matrix_BUSCO.csv"
		fpin = open(fNAME_BuscoCoreMatrix,'r')
		fpin.close()
	except:
...
for RefID in RefID_list:
	oPATH = "BUSCO_genoPlotR_input/"+RefID+"/"
	try:
